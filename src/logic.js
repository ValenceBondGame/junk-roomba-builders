import {VEHICLES,VEHICLE_ORDER,COMMON,SUBSTITUTES,MIN_PARTS,slotList} from './data.js';
...
export function findBestVehicle(items,target=null){const eligible=VEHICLE_ORDER.filter(key=>items.length>=MIN_PARTS[key]);if(!eligible.length)return null;return eligible.map((key,order)=>{const a=allocate(items,key,false);return {key,order,...a,exact:a.assignments.filter(x=>x.type==='exact').length,slots:a.assignments.length}}).sort((a,b)=>b.rate-a.rate||(b.key===target)-(a.key===target)||b.exact-a.exact||a.slots-b.slots||a.order-b.order)[0]}
...
export function buildVehicle(items,target=null){const best=findBestVehicle(items,target);if(!best)return null;let allocation=allocate(items,best.key,false);const useful=allocation.assignments.filter(x=>x.item).length;const minimum=MIN_PARTS[best.key];let mystery=useful<minimum;if(mystery)allocation=allocate(items,best.key,true);const chosen=allocation.assignments.filter(x=>x.item);if(chosen.length<minimum){const extra=items.filter(i=>!allocation.usedIds.includes(i.id)).slice(0,minimum-chosen.length);extra.forEach(i=>{const miss=allocation.assignments.find(a=>!a.item);if(miss){Object.assign(miss,{item:i,type:'junk',value:10,junk:.85});allocation.usedIds.push(i.id)}})}const used=allocation.assignments.filter(x=>x.item);return {id:'v'+Math.random().toString(36).slice(2),base:best.key,name:vehicleName(best.key,used,mystery),assignments:allocation.assignments,items:used.map(x=>x.item),usedIds:used.map(x=>x.item.id),mystery}}
