import {buildVehicle,scoreVehicle,removeUsed,allocate,addToInventory,findBestVehicle} from './logic.js';import {slotList,MIN_PARTS} from './data.js';
...
const sessionA={npc:{inventory:items(['本'])},listeners:1},sessionB={npc:{inventory:[]},listeners:1};console.assert(sessionA.npc!==sessionB.npc&&sessionB.npc.inventory.length===0&&sessionB.listeners===1,'14');
console.assert(MIN_PARTS.bicycle===5&&MIN_PARTS.helicopter===6&&MIN_PARTS.car===7&&MIN_PARTS.plane===8&&MIN_PARTS.train===10,'車種別最低数');console.assert(findBestVehicle(items(Array(4).fill('タイヤ')))===null,'最低5個');console.log('16件の開発用検証: OK');
